...
def main():
    ...
    parser.add_argument(
        "--scenesplat",
        action="store_true",
        help="download ScanNet files for SceneSplat 3DGS",
    )
    parser.add_argument(
        "--type",
    ...
    if args.v1:
        ...
        assert (not args.tf_semantic) and (not args.grit) and (not args.scenesplat), (
    ...
    if args.task_data:  # download task data
        ...
        download_file(
            os.path.join(BASE_URL, RELEASE_TASKS, MCMCGS_FILES[0]),
